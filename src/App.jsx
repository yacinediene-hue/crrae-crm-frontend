...
function Demandes({ onOpenCommentaires, onAssigner, ouvrirNouvelleDemande, onNouvelleDemandeOuverte, demandesInitiales = [], onDemandesChange, controlee = false }) {
  ...
  const handleEdit = (d) => {
    ...
    setForm({
      ...
      niveauTraitement: d.niveauTraitement || 1,
    })
    setShowForm(true)
    window.scrollTo(0, 0)
  }
    ...
    reader.onload = (ev) => {
      ...
      const lignes = rows
        ...
        .map(r => ({
          ...
          objetDemande: r['Objet de la demande'] || r['Objet'] || r['Type de demande'] || r['objetDemande'] || '',
          commentaire: r['Commentaire'] || r['commentaire'] || '',
          agentN1: r['Agent traitant N1'] || r['Agent N1'] || r['Agent'] || r['agentN1'] || '',
          service: r['Service'] || r['service'] || '',
          agentN2: r['Agent Traitant N2'] || r['Agent N2'] || r['agentN2'] || '',
          ...
          canalCommunication: r['Canal de communication'] || r['Canal communication'] || r['canalCommunication'] || '',
          noteSatisfaction: r['Note satisfaction (1-5)'] || r['Note satisfaction'] || r['noteSatisfaction'] || '',
        }))
        ...
        <>
          ...
          <h3 style={{color:'#1a365d',margin:'0.25rem 0 1rem',fontSize:'1rem',borderBottom:'1px solid #e2e8f0',paddingBottom:'0.5rem'}}>
            ...
            <span style={{marginLeft:'0.6rem',fontSize:'0.75rem',padding:'0.15rem 0.5rem',borderRadius:'4px',fontWeight:'700',
              ...
              {form.niveauTraitement===2 ? 'N2 — Back Office' : 'N1 — Front Office'}
            </span>
          </h3>
          <div style={col2}>
            ...
            <tbody>
              ...
              : filtered.map(d=>(
                ...
                >
                  ...
                  <td style={styles.td}>
                    {f(d.agentN1)}
                    {d.niveauTraitement === 2 && d.agentN2 && <div style={{fontSize:'0.75rem',color:'#6b46c1',marginTop:'0.2rem'}}>N2: {d.agentN2}</div>}
                  </td>
                  <td style={styles.td}>{f(d.service)}</td>
                  <td style={styles.td}>
                    <span style={{...styles.badge,...sColor(d.statut)}}>{f(d.statut)}</span>
                    {getSlaBadge(d)}
                    {d.niveauTraitement === 2
                      ? <span style={{display:'block',marginTop:'0.2rem',fontSize:'0.72rem',background:'#faf5ff',color:'#6b46c1',borderRadius:'4px',padding:'0.1rem 0.4rem',fontWeight:'700'}}>N2 — Back Office</span>
                      : <span style={{display:'block',marginTop:'0.2rem',fontSize:'0.72rem',background:'#ebf8ff',color:'#2b6cb0',borderRadius:'4px',padding:'0.1rem 0.4rem',fontWeight:'700'}}>N1 — Front Office</span>
                    }
                  </td>
                  <td style={styles.td}>
                    <span style={{
                  ...
                  <td style={{...styles.td, whiteSpace:'nowrap', position:'sticky', right:0, background:d.priorite==='Urgent'?'#fffaf0':'white', zIndex:1}}>
                    ...
                    {peutEnvoyerEnquete(d) && (
                      <button
                        ...
                        onClick={e => { e.stopPropagation(); envoyerEnquete(d) }}
...
function PageEnquetes() {
  ...
  useEffect(() => {
    API.get('/demandes/admin/stats-enquetes')
  ...
  const envoyees = demandes.filter(d => d.enqueteEnvoyee).length
  const repondues = demandes.filter(d => d.noteSatisfaction).length
  const tauxReponse = envoyees > 0 ? Math.round(repondues / envoyees * 100) : 0
  const notes = demandes.filter(d => d.noteSatisfaction).map(d => d.noteSatisfaction)
  const moyNote = notes.length > 0 ? (notes.reduce((a,b) => a+b,0) / notes.length).toFixed(1) : '—'
  ...
  const repartitionNotes = [1,2,3,4,5].map(n => ({
    note: `${n}★`, count: notes.filter(x => x === n).length
  }))

  // Filtres
  const listeAffichee = demandes.filter(d => {
    if (onglet === 'envoyees') return d.enqueteEnvoyee && !d.noteSatisfaction
    if (onglet === 'repondues') return !!d.noteSatisfaction
    if (onglet === 'non_envoyees') return !d.enqueteEnvoyee && ['Traité','Clôturé'].includes(d.statut)
    return true
  })

  // Import CSV/Excel
  const handleFichierImport = (e) => {
    const file = e.target.files[0]
    if (!file) return
    e.target.value = ''
    ...
    reader.onload = (ev) => {
      try {
        ...
        const parsed = rows
          .filter(r => Object.values(r).some(v => String(v).trim().length > 0))
          .map(r => ({
            numDemande: String(r['N° Demande'] || r['Numéro demande'] || r['numDemande'] || '').trim(),
            noteSatisfaction: r['Note satisfaction'] || r['Note'] || r['noteSatisfaction'] || '',
            avis: String(r['Avis'] || r['Commentaire'] || r['avis'] || '').trim(),
          }))
          .filter(r => r.numDemande)
        if (parsed.length === 0) { setImportErreur('Aucune ligne valide trouvée. Vérifiez les colonnes : N° Demande, Note satisfaction, Avis'); return }
        setImportRows(parsed)
      } catch (err) {
        setImportErreur('Erreur de lecture du fichier : ' + err.message)
      }
    }
    reader.readAsArrayBuffer(file)
  }
  ...
  const confirmerImport = async () => {
    ...
    setImportErreur(null)
    try {
      const res = await API.post('/demandes/admin/import-enquetes', { rows: importRows })
      const r = res.data
      setImportMsg(`Import terminé — ${r.updated} mis à jour sur ${r.total} lignes${r.notFound.length > 0 ? ` (${r.notFound.length} introuvable(s))` : ''}${r.errors.length > 0 ? ` — ${r.errors.length} erreur(s)` : ''}`)
      setImportRows(null)
      // Rafraichir les données
      API.get('/demandes/admin/stats-enquetes').then(r2 => setDemandes(r2.data)).catch(() => {})
    } catch (err) {
      setImportErreur('Erreur lors de l\'import : ' + (err?.response?.data?.message || err.message))
    } finally {
  ...
  const telechargerModele = () => {
    const ws = XLSX.utils.json_to_sheet([
      { 'N° Demande': 'DEMS-00001', 'Note satisfaction': 4, 'Avis': 'Bonne prise en charge' },
      { 'N° Demande': 'DEMS-00002', 'Note satisfaction': 5, 'Avis': '' },
    ])
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Enquêtes')
    XLSX.writeFile(wb, 'modele_import_enquetes.xlsx')
  }

  const s = {
    card: { background:'white', borderRadius:'10px', padding:'1.25rem 1.5rem', boxShadow:'0 1px 4px rgba(0,0,0,0.08)', minWidth:'140px' },
    label: { fontSize:'0.78rem', color:'#718096', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:'0.35rem' },
    val: { fontSize:'2rem', fontWeight:'700', color:'#1a365d', lineHeight:1 },
    sub: { fontSize:'0.82rem', color:'#718096', marginTop:'0.25rem' },
    tab: (active) => ({
      padding:'0.4rem 1rem', borderRadius:'20px', cursor:'pointer', fontSize:'0.85rem', fontWeight:'600', border:'none',
      background: active ? '#2b6cb0' : '#edf2f7', color: active ? 'white' : '#4a5568',
    }),
    th: { padding:'0.5rem 0.75rem', textAlign:'left', color:'#718096', fontSize:'0.78rem', fontWeight:'600', textTransform:'uppercase', letterSpacing:'0.04em', borderBottom:'2px solid #e2e8f0', background:'#f8fafc' },
    td: { padding:'0.5rem 0.75rem', color:'#2d3748', fontSize:'0.875rem', borderBottom:'1px solid #f0f4f8' },
  }

  const noteLabel = (n) => n ? ['','Très insatisfait','Insatisfait','Satisfait','Très satisfait','Excellent'][n] : '—'
  const noteCouleur = (n) => n >= 4 ? '#276749' : n === 3 ? '#b7791f' : n ? '#c53030' : '#718096'

  if (loading) return <div style={{padding:'3rem',textAlign:'center',color:'#718096'}}>Chargement...</div>

  return (
    <div>
      <h2 style={{color:'#1a365d',marginBottom:'1.25rem',fontSize:'1.125rem',fontWeight:'700'}}>Enquêtes de satisfaction</h2>

      {/* KPI cards */}
      <div style={{display:'flex',gap:'0.75rem',flexWrap:'wrap',marginBottom:'1.5rem'}}>
        <div style={s.card}><div style={s.label}>Demandes traitées</div><div style={s.val}>{total}</div></div>
        <div style={s.card}><div style={s.label}>Enquêtes envoyées</div><div style={{...s.val,color:'#2b6cb0'}}>{envoyees}</div></div>
        <div style={s.card}><div style={s.label}>Réponses reçues</div><div style={{...s.val,color:'#276749'}}>{repondues}</div><div style={s.sub}>Taux : {tauxReponse}%</div></div>
        <div style={s.card}><div style={s.label}>Note moyenne</div><div style={{...s.val,color: Number(moyNote)>=4?'#276749':Number(moyNote)>=3?'#b7791f':'#c53030'}}>{moyNote !== '—' ? `${moyNote}/5` : '—'}</div></div>
        {nps !== null && (
          <div style={s.card}>
            <div style={s.label}>Score satisfaction</div>
            <div style={{...s.val,color: nps>=50?'#276749':nps>=0?'#b7791f':'#c53030'}}>{nps > 0 ? '+' : ''}{nps}</div>
            <div style={s.sub}>{promoteurs}P · {passifs}N · {detracteurs}D</div>
          </div>
        )}
      </div>

      {/* Répartition des notes (mini barchart) */}
      {notes.length > 0 && (
        <div style={{background:'white',borderRadius:'10px',padding:'1.25rem',boxShadow:'0 1px 4px rgba(0,0,0,0.08)',marginBottom:'1.5rem'}}>
          <div style={{fontSize:'0.85rem',fontWeight:'700',color:'#2d3748',marginBottom:'1rem'}}>Répartition des notes</div>
          <div style={{display:'flex',gap:'0.5rem',alignItems:'flex-end',height:'80px'}}>
            {repartitionNotes.map(({note:label,count}) => {
              const pct = Math.max(count / Math.max(...repartitionNotes.map(x=>x.count)),0)
              const couleur = label.startsWith('4')||label.startsWith('5') ? '#48bb78' : label.startsWith('3') ? '#f6ad55' : '#fc8181'
              return (
                <div key={label} style={{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:'0.25rem'}}>
                  <span style={{fontSize:'0.75rem',color:'#718096',fontWeight:'600'}}>{count}</span>
                  <div style={{width:'100%',background:couleur,borderRadius:'4px 4px 0 0',height:`${Math.max(pct*60,count>0?6:0)}px`,transition:'height 0.3s'}} />
      ...
      {/* Import admin */}
      {isAdmin && (
        <div style={{background:'white',borderRadius:'10px',padding:'1.25rem',boxShadow:'0 1px 4px rgba(0,0,0,0.08)',marginBottom:'1.5rem',border:'1px solid #bee3f8'}}>
          <div style={{fontWeight:'700',color:'#2b6cb0',marginBottom:'0.75rem',fontSize:'0.92rem'}}>Importer les résultats d'enquête</div>
          <p style={{fontSize:'0.85rem',color:'#4a5568',marginBottom:'0.75rem',margin:'0 0 0.75rem 0'}}>
            Téléversez un fichier Excel/CSV avec les colonnes : <strong>N° Demande</strong>, <strong>Note satisfaction</strong> (1 à 5), <strong>Avis</strong> (optionnel).
          </p>
          <div style={{display:'flex',gap:'0.75rem',flexWrap:'wrap',alignItems:'center'}}>
            <button onClick={telechargerModele} style={{padding:'0.5rem 1rem',background:'#edf2f7',border:'1px solid #e2e8f0',borderRadius:'6px',cursor:'pointer',fontSize:'0.85rem',fontWeight:'600',color:'#4a5568'}}>
              Télécharger le modèle
            </button>
            <label style={{padding:'0.5rem 1rem',background:'#2b6cb0',color:'white',borderRadius:'6px',cursor:'pointer',fontSize:'0.85rem',fontWeight:'600'}}>
              Choisir un fichier
              <input type="file" accept=".xlsx,.xls,.csv" style={{display:'none'}} onChange={handleFichierImport} />
            </label>
          </div>

          {importErreur && <div style={{marginTop:'0.75rem',color:'#c53030',fontSize:'0.85rem',background:'#fff5f5',padding:'0.6rem 1rem',borderRadius:'6px'}}>{importErreur}</div>}
          {importMsg && <div style={{marginTop:'0.75rem',color:'#276749',fontSize:'0.85rem',background:'#f0fff4',padding:'0.6rem 1rem',borderRadius:'6px'}}>{importMsg}</div>}

          {importRows && (
            <div style={{marginTop:'1rem'}}>
              <div style={{fontSize:'0.85rem',fontWeight:'600',color:'#2d3748',marginBottom:'0.5rem'}}>{importRows.length} ligne(s) à importer — aperçu :</div>
              <div style={{overflowX:'auto',maxHeight:'220px',border:'1px solid #e2e8f0',borderRadius:'6px'}}>
                <table style={{width:'100%',borderCollapse:'collapse',fontSize:'0.82rem'}}>
                  <thead>
                    <tr>
                      <th style={s.th}>N° Demande</th>
                  ...
                  <tbody>
                    {importRows.slice(0,50).map((r,i) => {
                      const note = parseInt(r.noteSatisfaction, 10)
                      const valide = !isNaN(note) && note >= 1 && note <= 5
                      return (
                        <tr key={i}>
                          <td style={s.td}>{r.numDemande}</td>
                          <td style={{...s.td, color: valide ? noteCouleur(note) : '#c53030', fontWeight:'600'}}>{valide ? `${note}/5` : `⚠ ${r.noteSatisfaction}`}</td>
                          <td style={{...s.td, color:'#718096'}}>{r.avis || '—'}</td>
                          <td style={s.td}>{valide ? '✓' : '⚠ Note invalide'}</td>
                        </tr>
                      )
                    })}
                  </tbody>
              ...
              <div style={{display:'flex',gap:'0.75rem',marginTop:'0.75rem'}}>
                <button
                  ...
                  style={{padding:'0.5rem 1.25rem',background:'#276749',color:'white',border:'none',borderRadius:'6px',cursor:'pointer',fontSize:'0.85rem',fontWeight:'600'}}
                >
                  {importLoading ? 'Import en cours...' : `Confirmer l'import (${importRows.length} lignes)`}
                </button>
                <button onClick={() => setImportRows(null)} style={{padding:'0.5rem 1rem',background:'#edf2f7',border:'none',borderRadius:'6px',cursor:'pointer',fontSize:'0.85rem',color:'#4a5568'}}>
                  Annuler
                </button>
              </div>
            </div>
      ...
      <div style={{background:'white',borderRadius:'10px',boxShadow:'0 1px 4px rgba(0,0,0,0.08)',overflow:'hidden'}}>
        <div style={{padding:'1rem 1.25rem',borderBottom:'1px solid #e2e8f0',display:'flex',gap:'0.5rem',flexWrap:'wrap',alignItems:'center'}}>
          <span style={{fontWeight:'700',color:'#2d3748',marginRight:'0.5rem',fontSize:'0.9rem'}}>Liste</span>
          {[
            {id:'toutes',label:`Toutes (${total})`},
            {id:'repondues',label:`Répondues (${repondues})`},
            {id:'envoyees',label:`Envoyées sans réponse (${envoyees - repondues})`},
            {id:'non_envoyees',label:`Traitées sans enquête`},
          ].map(t => (
            <button key={t.id} style={s.tab(onglet===t.id)} onClick={() => setOnglet(t.id)}>{t.label}</button>
          ))}
        </div>
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead>
              <tr>
                ...
                <th style={s.th}>Agent N1</th>
                <th style={s.th}>Service</th>
                <th style={s.th}>Statut</th>
                <th style={s.th}>Enquête</th>
                <th style={s.th}>Note</th>
                <th style={s.th}>Avis</th>
              </tr>
            </thead>
            <tbody>
              {listeAffichee.length === 0 && (
                <tr><td colSpan={8} style={{...s.td,textAlign:'center',color:'#a0aec0',padding:'2rem'}}>Aucune demande dans cette catégorie</td></tr>
              )}
              {listeAffichee.map(d => (
                <tr key={d.numDemande}>
                  <td style={{...s.td,fontWeight:'600',color:'#2b6cb0'}}>{d.numDemande}</td>
                  <td style={s.td}>{d.nomPrenom}</td>
                  <td style={s.td}>{d.agentN1 || '—'}</td>
                  <td style={s.td}>{d.service || '—'}</td>
                  <td style={s.td}>
                    <span style={{
                      background: d.statut==='Traité'||d.statut==='Clôturé' ? '#f0fff4' : '#fffbeb',
                      color: d.statut==='Traité'||d.statut==='Clôturé' ? '#276749' : '#b7791f',
                      padding:'0.15rem 0.5rem', borderRadius:'12px', fontSize:'0.78rem', fontWeight:'600'
                    }}>{d.statut}</span>
                  </td>
                  <td style={s.td}>
                    {d.enqueteEnvoyee
                      ? <span style={{color:'#276749',fontSize:'0.82rem',fontWeight:'600'}}>Envoyée{d.dateEnvoiEnquete ? ` le ${new Date(d.dateEnvoiEnquete).toLocaleDateString('fr-FR')}` : ''}</span>
                      : <span style={{color:'#a0aec0',fontSize:'0.82rem'}}>Non envoyée</span>
                    }
                  </td>
                  <td style={{...s.td, fontWeight:'700', color: d.noteSatisfaction ? noteCouleur(d.noteSatisfaction) : '#a0aec0'}}>
                    {d.noteSatisfaction ? `${d.noteSatisfaction}/5` : '—'}
                  </td>
                  <td style={{...s.td, color:'#718096', fontSize:'0.82rem', maxWidth:'200px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap'}}>
                    {d.noteSatisfaction ? noteLabel(d.noteSatisfaction) : '—'}
                  </td>
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div style={styles.layout}>
      <nav style={styles.nav}>
        ...
        <NavLink style={({ isActive }) => ({ ...styles.navLink, background: isActive ? 'rgba(255,255,255,0.15)' : 'transparent' })} to="/enquetes"><span style={styles.sidebarIcon}>⭐</span>Enquêtes</NavLink>
    ...
    <BrowserRouter>
      ...
      >
        ...
        <Routes>
          ...
          <Route path="/enquetes" element={['admin','manager'].includes(localStorage.getItem('userRole')) ? <PageEnquetes /> : <Navigate to="/accueil" />} />
